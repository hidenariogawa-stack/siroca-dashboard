...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (type === 'devices') {
      ...
      const cvMap = {};
      (cvRes.rows || []).forEach(r => {
        cvMap[r.dimensionValues[0].value] = parseInt(r.metricValues[0].value) || 0;
      });

      return {
        statusCode: 200, headers,
        body: JSON.stringify({ rows: response.rows || [], cvMap })
      };
    }
